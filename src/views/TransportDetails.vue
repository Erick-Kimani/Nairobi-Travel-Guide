...
const numericPrice = computed(() => {
  ...
  const num = parseFloat(String(transport.value.price).replace(/[^0-9.]/g, ''))
  return isNaN(num) ? 0 : num
})

async function handleVisit() {
  if (!transport.value?.website) return
  try {
    await api.post('/track-click', {
      service_id:   transport.value.id,
      service_name: transport.value.name,
      name:         currentUser.value?.name ?? 'Guest',
    })
  } catch (error) {
    console.error('Transport click tracking failed:', error)
  } finally {
    ...
    <v-row no-gutters class="fill-height min-h-screen">
      ...
      <v-col cols="12" md="6" lg="5" class="relative overflow-hidden">
        <div class="full-page-inset">
          <v-img :src="transport?.image" cover height="100vh" class="main-image">
      ...
      <v-col cols="12" md="6" lg="7" class="d-flex align-center bg-white">
        <v-container class="pa-10 pa-md-16">
          <div v-if="transport" class="content-wrapper">
            <span class="text-overline text-blue font-weight-bold mb-2 d-block">Premium Transport</span>
            ...
            <div class="pt-6">
              ...
              <button v-if="transport.website" @click="handleVisit" class="item-link" type="button">
