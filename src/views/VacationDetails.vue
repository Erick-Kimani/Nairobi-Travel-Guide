...
const numericPrice = computed(() => {
  ...
  const num = parseFloat(String(vacation.value.price).replace(/[^0-9.]/g, ''))
  return isNaN(num) ? 0 : num
})

async function handleVisit() {
  if (!vacation.value?.website) return
  try {
    await api.post('/track-click', {
      service_id:   vacation.value.id,
      service_name: vacation.value.name,
      name:         currentUser.value?.name ?? 'Guest',
    })
  } catch (error) {
    console.error('Vacation click tracking failed:', error)
  } finally {
...
<template>
  <div class="detail-root">
    ...
    <div class="hero">
      ...
      <div class="hero-topbar">
        ...
        <div class="hero-breadcrumb">
          <span>Vacation Hotels</span>
          <span class="bc-sep">›</span>
          <span class="bc-current">{{ vacation?.name }}</span>
        </div>
      </div>

      <!-- ── bottom title block ── -->
      <div class="hero-bottom" v-if="vacation">

        <!-- star rating row -->
        <div class="star-row">
          <v-icon v-for="n in 5" :key="n" size="14" class="star-icon">mdi-star</v-icon>
          <span class="star-label">5-Star Experience</span>
        </div>

        <h1 class="hero-title">{{ vacation.name }}</h1>
        ...
        <div class="hero-tags">
          ...
          <span class="htag">
            <v-icon size="12">mdi-crown</v-icon> Luxury
          </span>
          <span class="htag">
            <v-icon size="12">mdi-leaf</v-icon> Nature
          </span>
        </div>
      </div>
    ...
    <!-- ░░░ LIFTED CONTENT ░░░ -->
    <div class="content-lift" v-if="vacation">
      <div class="content-card">
        ...
        <div class="card-inner">
          ...
          <div class="card-text">
            ...
            <p class="card-desc">{{ vacation.description }}</p>

            <!-- highlights row -->
            <div class="highlights">
              <div class="highlight-item">
                <div class="hi-icon-wrap">
                  <v-icon size="18" class="hi-icon">mdi-bed-outline</v-icon>
                </div>
                <div>
                  <div class="hi-val">Luxury</div>
                  <div class="hi-key">Rooms</div>
                </div>
              </div>
              <div class="highlight-item">
                <div class="hi-icon-wrap">
                  <v-icon size="18" class="hi-icon">mdi-silverware-fork-knife</v-icon>
                </div>
                <div>
                  <div class="hi-val">Fine</div>
                  <div class="hi-key">Dining</div>
                </div>
              </div>
              <div class="highlight-item">
                <div class="hi-icon-wrap">
                  <v-icon size="18" class="hi-icon">mdi-pool</v-icon>
                </div>
                <div>
                  <div class="hi-val">Pool</div>
                  <div class="hi-key">& Spa</div>
                </div>
              </div>
              <div class="highlight-item">
                <div class="hi-icon-wrap">
                  <v-icon size="18" class="hi-icon">mdi-wifi</v-icon>
                </div>
                <div>
                  <div class="hi-val">Free</div>
                  <div class="hi-key">Wi-Fi</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card-action">
            ...
            <div class="trust-badges">
              ...
              <div class="trust-badge">
                <v-icon size="15" class="tb-icon">mdi-clock-outline</v-icon>
...
<style scoped>
/* ── Google Font import (Cormorant Garamond for display elegance) ── */
...
/*
 * Four-layer scrim:
 *  1. Strong bottom band  → anchors title
 *  2. Soft top fade       → keeps sky detail
 *  3. Left edge shadow    → back-btn contrast
 *  4. Warm amber tint     → makes image feel golden-hour
 */
.hero-scrim {
  position: absolute;
  inset: 0;
...
/* Big title — Cormorant for that luxury editorial feel */
.hero-title {
...
/* CTA — looks like a real button, not just text */
.visit-btn {
  display: flex;
  align-items: center;
  justify-content: space-between;
