    ...
    <v-row no-gutters class="fill-height min-h-screen">
      ...
      <v-col cols="12" md="6" lg="5" class="relative overflow-hidden">
        <div class="full-page-inset">
          <v-img :src="transfer?.image" cover height="95vh" max width="100%" class="main-image">
      ...
      <v-col cols="12" md="6" lg="7" class="d-flex align-center bg-white">
        <v-container class="pa-10 pa-md-16">
          <div v-if="transfer" class="content-wrapper">
            <span class="text-overline text-blue font-weight-bold mb-2 d-block">Premium Transfer</span>
            <h1 class="text-h2 font-weight-black text-grey-darken-4 mb-6 leading-tight">{{ transfer.name }}</h1>
            <p class="text-h6 text-grey-darken-1 mb-8 leading-relaxed">{{ transfer.description }}</p>
            <v-divider class="mb-8" />
           
            <v-divider class="mb-8" />
            <div class="pt-6">
              <div class="text-caption text-grey-lighten-1 mb-2">OFFICIAL WEBSITE</div>
              <button v-if="transfer.website" @click="handleVisit" class="item-link" type="button">
