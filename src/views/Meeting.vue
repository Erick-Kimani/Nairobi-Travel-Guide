...
const filteredMeetings = computed(() => {
  if (!searchQuery.value) return store.meetings
  ...
  <div class="page-container">
    <div class="hero-header">
      <div class="hero-image-container">
        <img src="/images/Picture 28 .jpg" alt="Header Image" class="hero-image" />
    ...
    <v-container class="py-10 content-body" fluid>
      <div class="search-wrapper mb-10 px-4">
        <v-text-field
          v-model="searchQuery"
          label="Search for a Meeting Room..."
      ...
      <div v-if="store.loading" class="text-center py-10">
        ...
        <p class="text-grey-lighten-1 mt-4">Loading meeting rooms…</p>
      </div>

      <div v-else-if="store.error" class="text-center py-10">
        <p class="text-red-lighten-1">{{ store.error }}</p>
        <v-btn color="orange" class="mt-4" @click="store.fetchMeetings()">Retry</v-btn>
      </div>
      ...
      <v-row v-else class="px-4">
        ...
        >
          <div class="card-border-wrapper">
            <v-card class="item-card" elevation="0" @click="viewDetails(meeting)">
      ...
      <v-row v-if="!store.loading && filteredMeetings.length === 0">
        <v-col cols="12" class="text-center">
          <p class="text-grey-lighten-1 mt-6">
            <template v-if="searchQuery">
              No meeting rooms found matching "<strong>{{ searchQuery }}</strong>"
            </template>
            <template v-else>No meeting rooms are available right now.</template>
          </p>
        </v-col>
      </v-row>
    </v-container>
