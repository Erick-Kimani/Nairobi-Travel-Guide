  ...
  <div class="page-container">
    ...
    <v-container class="py-10 content-body" fluid>
      <div class="search-wrapper mb-10 px-4">
        <v-text-field
          v-model="searchQuery"
          label="Search for a Service Apartment..."
      ...
      <div v-if="store.loading" class="text-center py-10">
        ...
        <p class="text-grey-lighten-1 mt-4">Loading service apartments…</p>
      </div>

      <div v-else-if="store.error" class="text-center py-10">
        <p class="text-red-lighten-1">{{ store.error }}</p>
        <v-btn color="orange" class="mt-4" @click="store.fetchServices()">Retry</v-btn>
      </div>
      ...
      <v-row v-else class="px-4">
        ...
        >
          <div class="card-border-wrapper">
            <v-card class="item-card" elevation="0" @click="viewDetails(service)">
      ...
      <v-row v-if="!store.loading && filteredServices.length === 0">
        <v-col cols="12" class="text-center">
          <p class="text-grey-lighten-1 mt-6">
            <template v-if="searchQuery">
              No service apartments found matching "<strong>{{ searchQuery }}</strong>"
            </template>
            <template v-else>No service apartments are available right now.</template>
          </p>
        </v-col>
      </v-row>
    </v-container>
