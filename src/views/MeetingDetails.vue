...
const numericPrice = computed(() => {
  ...
  const num = parseFloat(String(meeting.value.price).replace(/[^0-9.]/g, ''))
    ...
    <v-row no-gutters class="fill-height min-h-screen">
      ...
      <v-col cols="12" md="6" lg="5" class="relative overflow-hidden">
        <div class="full-page-inset">
          <v-img :src="meeting?.image" cover height="100vh" class="main-image">
      ...
      <v-col cols="12" md="6" lg="7" class="d-flex align-center bg-white">
        <v-container class="pa-10 pa-md-16">
          <div v-if="meeting" class="content-wrapper">
            <span class="text-overline text-blue font-weight-bold mb-2 d-block">Premium Space</span>
            <h1 class="text-h2 font-weight-black text-grey-darken-4 mb-6 leading-tight">{{ meeting.name }}</h1>
            <p class="text-h6 text-grey-darken-1 mb-8 leading-relaxed">{{ meeting.description }}</p>
            <v-divider class="mb-8" />
            <div class="mb-10">
              <div class="text-subtitle-1 text-grey-lighten-1 mb-1">Daily Rate</div>
            ...
            <div class="pt-6">
              ...
              <button v-if="meeting.website" @click="handleVisit" class="item-link" type="button">
