...
const filteredTransfers = computed(() => {
  if (!searchQuery.value) return store.transfers
...
function viewDetails(transfer) {
  store.selectTransfer(transfer)
  router.push('/airportdetails')
}
</script>
  ...
  <div class="page-container">
    ...
    <v-container class="py-10 content-body" fluid>
      ...
      <div v-if="store.loading" class="text-center py-10">
        ...
        <p class="text-grey-lighten-1 mt-4">Loading Airport Transfers…</p>
      </div>

      <div v-else-if="store.error" class="text-center py-10">
        <p class="text-red-lighten-1">{{ store.error }}</p>
        <v-btn color="orange" class="mt-4" @click="store.fetchTransfers()">Retry</v-btn>
      </div>
      ...
      <v-row v-else class="px-4">
        ...
        >
          <div class="card-border-wrapper">
            <v-card class="item-card" elevation="0" @click="viewDetails(transfer)">
      ...
      <v-row v-if="!store.loading && filteredTransfers.length === 0">
        <v-col cols="12" class="text-center">
          <p class="text-grey-lighten-1 mt-6">
            <template v-if="searchQuery">
              No transfers found matching "<strong>{{ searchQuery }}</strong>"
            </template>
            <template v-else>No transfers are available right now.</template>
          </p>
        </v-col>
      </v-row>
    </v-container>
...
.hero-image { width: 100%; height: 149%; object-fit: cover; }
